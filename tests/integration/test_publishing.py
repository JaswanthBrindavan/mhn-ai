...
def test_submitted_item_is_queued_and_published(api, make_report, aws):
    _, sqs, queue_url, _ = aws
    report_id = make_report()

    body = api.post("/v1/report-processing-runs", json={"report_ids": [report_id]}).json()
...
def test_message_contains_no_s3_key_or_report_data(api, make_report, aws):
    """The queue must never become a second copy of patient data."""
    _, sqs, queue_url, _ = aws
    api.post("/v1/report-processing-runs", json={"report_ids": [make_report()]})

    payload = json.loads(_drain(sqs, queue_url)[0]["Body"])

    assert set(payload) == {"schema_version", "item_id", "run_id", "report_id", "attempt"}


def test_each_report_gets_exactly_one_message(api, make_report, aws):
    _, sqs, queue_url, _ = aws
    report_ids = [make_report(), make_report(), make_report()]

    api.post("/v1/report-processing-runs", json={"report_ids": report_ids})

    assert len(_drain(sqs, queue_url)) == 3


def test_reused_item_is_not_published_twice(api, make_report, aws):
...
def test_rejected_item_is_never_published(api, make_report, aws):
    _, sqs, queue_url, _ = aws
    bad = make_report(content_type="application/msword", suffix=".docx")

    body = api.post("/v1/report-processing-runs", json={"report_ids": [bad]}).json()
...
def test_missing_s3_object_is_rejected_with_a_reason(api, make_report):
...
def test_oversized_file_is_rejected(api, make_report, test_settings):
    big = make_report(body=b"x" * (test_settings.max_file_bytes + 1))

    body = api.post("/v1/report-processing-runs", json={"report_ids": [big]}).json()

    assert body["items"][0]["error_code"] == "file_too_large"


def test_unsupported_type_is_rejected(api, make_report):
    bad = make_report(content_type="application/msword", suffix=".docx")
...
def test_octet_stream_pdf_is_accepted(api, make_report, aws):
    """Uploads often lose their content type; the extension must save them."""
    report_id = make_report(content_type="application/octet-stream", suffix=".pdf")
...
def test_one_bad_report_does_not_block_the_rest_of_the_batch(api, make_report, aws):
    _, sqs, queue_url, _ = aws
    good_a, bad, good_b = make_report(), make_report(upload=False), make_report()

    body = api.post("/v1/report-processing-runs", json={"report_ids": [good_a, bad, good_b]}).json()

    statuses = {item["report_id"]: item["status"] for item in body["items"]}
...
def test_rejected_report_can_be_resubmitted_after_the_file_is_fixed(
    ...
    key = db_session.execute(
        text("SELECT filepath FROM reports WHERE id = :id"), {"id": report_id}
    ).scalar_one()
...
def test_publish_failure_leaves_the_item_pending(api, make_report, aws, db_session):
