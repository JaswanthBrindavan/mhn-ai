...
def test_submit_rejects_unknown_report_ids(api, make_document):
    ...
    assert body["error"]["details"]["missing_document_ids"] == [999_000_111]


def test_duplicate_ids_within_one_request_create_one_item(api, make_document):
    document_id = make_document()
    response = api.post(
        "/v1/document-processing-runs",
...
def test_family_upload_is_accepted(api, make_document, make_user):
    ...
    document_id = make_document(created_by=relative)
...
def test_mismatched_requesting_user_still_processes(api, make_document):
...
def test_get_unknown_run_is_404(api):
    response = api.get(f"/v1/document-processing-runs/{uuid.uuid4()}")
    assert response.status_code == 404
...
def test_cancel_marks_in_flight_items_cancelled(api, make_document):
    ...
    after = api.get(f"/v1/document-processing-runs/{run['run_id']}").json()
